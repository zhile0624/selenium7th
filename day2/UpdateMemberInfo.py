...
# 5.修改性别
# 通过观察,可以发现,保密,男,女三者唯一的区别就是value属性的值不同
# 那么我们可不可以通过value属性来定位?
# 可以, 实际上我们可以通过任何属性来定位.
# 要想通过value属性定位有两种方法: xpath和css_selector
# 通过cssselectot定位元素,只需要在唯一属性的两边加一对中括号即可
driver.find_element_by_css_selector('[value="2"]').click()
# 在xpath中//表示采用相对路径定位元素,
# /单斜杠表示绝对路径, 一般都是从/html跟节点开始定位元素
# 相对路径一般通过元素的特殊属性查找元素
...
# 一下一下点年,月,日是可以实现的,
# 但是稳定性比较差, 很容易点错
# 并且很难修改日期,比如写完1990-02-02,下次想换个日期,还需要重新一个一个定位,
# 所以尽量不要用click()点击选日期
# 我们右键检查,可以发现日历控件其实是一个文本输入框
# 那么我们可不可以用send_keys的方法来输入一个日期?
# 日历控件明明是一个输入框,为什么不能输入?
# 因为readonly属性, 写一个javascript,删除readonly属性
driver.execute_script('document.getElementById("date").removeAttribute("readonly")')
driver.find_element_by_id("date").clear()
driver.find_element_by_id("date").send_keys("1950-05-04")
# 7.修改QQ
# 8.点击确定,保存成功
# driver.find_element(确定按钮).click()
# time.sleep(3)
